...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid or event.query.user_id in Config.SUDO_USERS:
            veriler = custom.Button.inline(f"⚡ Re-Open Menu ⚡", data="page")
            await event.edit(f"**⚜️ rebelbot Mêñû Prõvîdêr háš běěn čłøšĕd ⚜️**\n\n**Bot Of :**  REBELBOT \n\n            © REBELBOT", buttons=veriler, link_preview=False)
        else:
            reply_pop_up_alert = "HELLO THERE. PLEASE MAKE YOUR OWN REBELBOT AND USE. © REBELBOT"
